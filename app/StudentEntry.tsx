...
export default function StudentEntry({ session: account }: { session: Session }) {
  ...
  const submit=async()=>{setState("saving");try{
    ...
    if(!response.ok){
      ...
      setState(corpo.error==="already_pending"?"pendente":"error");return;
    }
    await load();setState("idle")}catch{setState("error")}};
